...
export function LoginClient({ referredByCode, nextPath }) {
  ...
  const loading = signingIn || isPending;
  ...
  async function handleLogin() {
    ...
    // Route by the signed-in user's role. Demo personas have a tailored `home`
    // (e.g. the agency_client needs its portal slug); fall back to a role-based
    // default for real signed-up users.
    const matchedDemo = Object.values(DEMO_USERS).find(
      (u) => u.email.toLowerCase() === email.trim().toLowerCase(),
    );
    const role = data?.user?.user_metadata?.role ?? matchedDemo?.role ?? null;
    const ROLE_HOME = {
      admin: "/admin",
      agency: "/agency/dashboard",
      referral_partner: "/referral/dashboard",
      direct_client: "/direct/dashboard",
    };
    // `?next=` from the invite email wins over role-based defaults so a
    // freshly-invited client lands straight on their portal. Otherwise use the
    // demo persona's home, then the role's default home, and finally fall
    // through to /post-login which resolves the destination server-side
    // (handles agency_client whose home depends on DB-resolved slugs).
    const destination = nextPath ?? matchedDemo?.home ?? ROLE_HOME[role] ?? "/post-login";
  ...
  return (
    ...
    >
      ...
      {referredByCode && (
        <div
          className="relative max-w-[420px] mb-4 px-4 py-2.5 rounded-[10px] text-[0.82rem] text-center"
          style={{
        ...
        >
          🤝 You were referred by <strong className="text-white">{referredByCode}</strong> — sign up below and they earn commission for 12 months.
        </div>
      )}
      ...
      <p className="text-center text-[0.82rem] text-white/55 mt-5">
        Don&apos;t have an account?{" "}
        <Link href="/signup" className="text-teal font-semibold hover:underline">
          Sign up as agency or direct client
        </Link>
      </p>

      <p className="text-center text-[0.7rem] text-white/20 mt-3 max-w-[420px] leading-relaxed">
        This is an interactive prototype. Demo credentials are pre-filled — sign-up flow is live for new accounts.
      </p>
    </div>
  );
}

function LoginField({ label, value, onChange, type, autoComplete, disabled }) {
  return (
    <div className="mb-3.5">
      ...
      <input
        ...
        disabled={disabled}
        className="w-full px-3.5 py-2.5 rounded-[10px] text-[0.88rem] font-body text-white outline-none transition-colors focus:border-teal disabled:opacity-60"
        style={{
